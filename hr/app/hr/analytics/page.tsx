...
export default function AnalyticsPage() {
	...
	const [recruitmentAnalyticsData, setRecruitmentAnalyticsData] = useState<RecruitmentAnalytics | null>(null);
	const [isLoadingRecruitmentAnalytics, setIsLoadingRecruitmentAnalytics] = useState(true);
	...
	useEffect(() => {
		...
		const fetchRecruitmentAnalytics = async () => {
			try {
				setIsLoadingRecruitmentAnalytics(true);
				const data = await getRecruitmentAnalytics(parseInt(orgId, 10));
				setRecruitmentAnalyticsData(data);
			} catch (error) {
				toast({
					title: "Error",
					description:
						error instanceof Error
							? error.message
							: "Failed to fetch recruitment analytics data",
					variant: "destructive"
				});
			} finally {
				setIsLoadingRecruitmentAnalytics(false);
			}
		};
	...
	return (
		<div className="p-6 space-y-8">
			...
			<div className="space-y-6">
				<div className="border-b pb-4">
					<h2 className="text-xl font-bold">Recruitment Analytics</h2>
				</div>
				<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
					<RecruitmentFunnelChart
						data={recruitmentAnalyticsData as RecruitmentAnalytics}
						isLoading={isLoadingRecruitmentAnalytics}
					/>
					<RecruitmentTrendChart
						data={recruitmentAnalyticsData as RecruitmentAnalytics}
						isLoading={isLoadingRecruitmentAnalytics}
