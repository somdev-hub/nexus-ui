...
import { TeamMember, AddTeamMemberRequest, TeamRole, User } from "@/types";
import { X, Loader2, Users, UserCheck, UserCog, User } from "lucide-react";
...
export function AddMemberForm({
    ...
    const filteredUsers = availableUsers.filter(
        (user) => !teamMembers.some((m) => m.user.id === user.id) || (isEditing && m.user.id === editingMember?.user.id)
    );
