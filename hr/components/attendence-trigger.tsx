...
import { AttendanceQuickUpdateResponse, ToggleAttendanceResponse } from '@/types';
...
const AttendenceTrigger = () => {
	...
	const loadAttendance = useCallback(async () => {
		// Guard: wait until auth is resolved and userId is available
	...
	}, [toast, userId, isUserLoading, isAuthenticated]);
	...
	const handleToggleAttendance = async () => {
		if (!userId) {
			toast({
				...
				description: "Please log in to update attendance",
				variant: "destructive"
			});
			return;
		}
		...
		if (!Number.isFinite(numericId)) {
			toast({
				title: "Invalid user",
				description: `Invalid userId: ${String(userId)}`,
				variant: "destructive"
			});
			return;
		}
