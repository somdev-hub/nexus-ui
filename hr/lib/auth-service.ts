...
export async function triggerEventMail(triggerRequest: TriggerEventMailRequest): Promise<AxiosResponse<string | { message: string }>> {
    try {
        const response = await apiClient.post<string | { message: string }>(
...
export async function getEventHitsMonthWise(templateName: string, orgId: number): Promise<Record<string, number>> {
    try {
...
export async function getEventHitsStatusWise(templateName: string, orgId: number): Promise<Record<string, number>> {
    try {
        const response = await apiClient.get<Record<string, number>>(
