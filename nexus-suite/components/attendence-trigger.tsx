...
import { getAttendanceQuickUpdate, toggleAttendance } from "@/lib/services/attendance-service";
...
const AttendenceTrigger = () => {
	...
	const { userId, isLoading: isUserLoading, isAuthenticated } = useUserMetadata();
	const loadAttendance = useCallback(async () => {
		if (isUserLoading || !isAuthenticated || !userId) {
			return;
		}
		const numericId = Number(userId);
		if (!Number.isFinite(numericId)) {
			console.warn("[SiteHeader] Invalid userId, skipping attendance load:", userId);
			return;
		}
		try {
			setIsLoadingAttendance(true);
			const data = await getAttendanceQuickUpdate(numericId);
			setAttendance(data);
		} catch (error) {
			console.error(
			...
			toast.error(error instanceof Error ? error.message : "Unable to load attendance details");
			setAttendance(null);
		} finally {
			setIsLoadingAttendance(false);
		}
	}, [userId, isUserLoading, isAuthenticated]);
	...
	}, [loadAttendance, isUserLoading, isAuthenticated, userId]);

	const handleToggleAttendance = async () => {
		if (!userId) {
			toast.error("Not authenticated - Please log in to update attendance");
			return;
		}
		const numericId = Number(userId);
		if (!Number.isFinite(numericId)) {
			toast.error(`Invalid userId: ${String(userId)}`);
			return;
		}
		try {
			setIsTogglingAttendance(true);
			const response = await toggleAttendance(numericId);
...
export default AttendenceTrigger
