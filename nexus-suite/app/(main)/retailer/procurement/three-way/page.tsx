...
export default function ThreeWayPage(){
  ...
  return (
    <div className="flex flex-1 flex-col p-6 gap-6">
      ...
      <Card>
        <CardHeader><CardTitle>Check Purchase Order</CardTitle></CardHeader>
      ...
      {result && (
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <Card>
            <CardHeader><CardTitle>Can Invoice?</CardTitle></CardHeader>
            <CardContent>{result.can ? <Badge className={result.can.canInvoice?"bg-green-100 text-green-800":"bg-red-100 text-red-800"}>{String(result.can.canInvoice)}</Badge> : "—"}</CardContent>
          </Card>
        </div>
      )}
    </div>
