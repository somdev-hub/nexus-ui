...
async function handleUnauthorizedWithRetry(
  sessionToken: string,
  refreshToken: string | undefined,
  retryFn: (accessToken: string) => Promise<unknown>
): Promise<{ success: boolean; data?: unknown; response?: NextResponse }> {
  if (!refreshToken) {
...
export async function PUT(request: NextRequest) {
  ...
  if (!session) {
    console.error(
      "[API PROXY PUT] Session not found for token:",
      sessionToken
    );
    return NextResponse.json({ error: "Session expired" }, { status: 401 });
  }
  ...
  try {
    ...
    const springBootClient = getSpringBootClient();
    const response = await springBootClient.put(path, body, axiosConfig);
...
export async function DELETE(request: NextRequest) {
  ...
  console.log("[API PROXY DELETE] SessionToken from cookie:", sessionToken);

  if (!sessionToken) {
    console.error("[API PROXY DELETE] No sessionToken in cookies");
    return NextResponse.json({ error: "Not authenticated" }, { status: 401 });
  }
  ...
  console.log("[API PROXY DELETE] Session found:", !!session);
  if (session) {
    console.log("[API PROXY DELETE] Session user:", session.user?.email);
    console.log("[API PROXY DELETE] Session expires at:", session.expiresAt);
  }

  if (!session) {
    console.error(
      "[API PROXY DELETE] Session not found for token:",
      sessionToken
    );
    return NextResponse.json({ error: "Session expired" }, { status: 401 });
  }
