...
export function useUserMetadata(): UserMetadata {
  ...
  return useMemo(() => ({
    ...
    isLoading,
    sessionExpiresAt
  }), [user?.id, user?.personalEmail, user?.name, user?.phone, user?.role, user?.avatar, isAuthenticated, isLoading, sessionExpiresAt]);
}
 ...
 *   const personalEmail = useUserEmail();
 *
 *   return <div>Contact: {personalEmail || "No email"}</div>;
 * }
 * ```
 */
export function useUserEmail(): string | undefined {
  const { personalEmail } = useUserMetadata();
