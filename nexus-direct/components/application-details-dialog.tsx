...
import { ApplicantApplicationSchema, ApplicationDetailsWithStatusHistory, ApplicationStatus, RecruitmentInterview } from '@/types';
		...
		return (
			<Dialog open={open} onOpenChange={onOpenChange}>
				<DialogContent className="max-w-4xl">
					...
					<div className="space-y-6 overflow-y-auto max-h-[90dvh] pr-2">
	...
	return (
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="max-w-4xl">
				...
				<div className="space-y-6 overflow-y-auto max-h-[80dvh] pr-2">
					{/* Role & Organization */}
					<div className="space-y-3">
					...
					{currentStatus === 'INTERVIEW_SCHEDULED' && details?.interviews && details.interviews.length > 0 && (
						<>
							<Separator />
							<div className="space-y-1">
								<h4 className="text-sm font-medium flex items-center gap-1.5 mb-3">
								...
								{details.interviews.map((interview: RecruitmentInterview) => (
									<div key={interview.recruitmentInterviewId} className="p-4 rounded-lg bg-purple-50 dark:bg-purple-950/20 border border-purple-200 dark:border-purple-800">
										<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
											...
											<div className="space-y-2">
												...
												<p className="text-sm font-medium">{interview.interviewMode}</p>
											</div>
											<div className="space-y-2">
												<p className="text-xs text-muted-foreground flex items-center gap-1">
													<Calendar className="h-3 w-3" />
												...
												<p className="text-sm font-medium">{new Date(interview.interviewDate).toLocaleDateString('en-US', {
													month: 'long',
													day: 'numeric',
													year: 'numeric',
												})}</p>
											...
											<div className="space-y-2">
												...
												<p className="text-sm font-medium">{interview.interviewTime}</p>
											</div>
											<div className="space-y-2">
												<p className="text-xs text-muted-foreground flex items-center gap-1">
													<Clock className="h-3 w-3" />
												...
												<p className="text-sm font-medium">{interview.interviewDuration} minutes</p>
											</div>
											<div className="space-y-2">
												<p className="text-xs text-muted-foreground flex items-center gap-1">
												...
												<p className="text-sm font-medium">{interview.interviewLocation}</p>
											</div>
											<div className="space-y-2">
												<p className="text-xs text-muted-foreground flex items-center gap-1">
												...
												<p className="text-sm font-medium">{interview.interviewerName || 'TBD'}</p>
											</div>
											<div className="space-y-2 md:col-span-2">
												...
												<a href={interview.interviewUrl} target="_blank" rel="noopener noreferrer" className="text-sm font-medium text-blue-600 hover:underline break-all flex items-center gap-1">
													{interview.interviewUrl}
													<Link2 className="h-3 w-3" />
												</a>
											</div>
											{interview.interviewerRemarks && (
												<div className="space-y-2 md:col-span-2">
													<p className="text-xs text-muted-foreground flex items-center gap-1">
														<FileText className="h-3 w-3" />
														Interviewer Remarks
													</p>
													<p className="text-sm font-medium">{interview.interviewerRemarks}</p>
												</div>
											)}
											<div className="space-y-2 md:col-span-2">
												<p className="text-xs text-muted-foreground flex items-center gap-1">
												...
												<p className="text-sm font-medium">{new Date(interview.interviewConfirmationDeadline).toLocaleString()}</p>
											</div>
										</div>
									</div>
								))}
